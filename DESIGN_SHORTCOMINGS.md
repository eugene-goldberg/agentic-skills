...
- [ ] **A49** *(filed 2026-08-15, audit session)* — Dependency DAG orders but never gates: with `stop_on_failure=False`, a BL whose dependency failed is dispatched anyway. **Class:** `silent-failure` · **Invariant:** I-5. **Evidence:** `orchestrator.py:1869` iterates `_dep_order(items)` and `continue`s past failures (`:1936`) with no check that a BL's `dependencies` are in the merged set — dependents build on missing foundations, burning a full engineer+gate cycle on guaranteed-confused work. The DAG (`_dep_order`, `:1731`) is consulted for *ordering only*. **Fix:** Batch 3-1 of `AUTONOMY_HARDENING_PLAN.md` — maintain `merged` set; unmet deps → `bl.skipped kind=dep_unmet`, outcome `deferred_dep`; sprint summary gains worst-wins `deferred` section. **Priority:** high (paired with the abort-default, this is why one bad BL costs the whole sprint either way).
- [ ] **A50** *(filed 2026-08-15)* — Quality controls fire after merge and have no teeth: scorer's numeric score/verdict never enters control flow; no revert primitive exists anywhere. **Class:** `silent-failure` · **Invariant:** I-5. **Evidence:** engineer FF-merges at `orchestrator.py:499–541`; QA (`:1943`) and scorer (`:2007`) run *after*; `bl.done` outcome consults only `doctrine_ok` (`:2024–2030`) — a merged BL scoring 40/100 with an honest **Fail** verdict yields `merged_full`. `validate_scorer` (`doctrine_validator.py:612–644`) parses dims solely for self-consistency. Once merged, the only remedy is forward-fix (ABL-0015, cap 1, flag-OFF); agents are R13-barred from history edits and the orchestrator owns no revert. Reindex-after-merge (`:1939`) then embeds defective code as retrieval "convention" — grounding amplifies the mistake. **Fix:** Batch 4 of `AUTONOMY_HARDENING_PLAN.md` (verdict→triage routing + operator-gated `revert_bl_span`). **Priority:** high.
- [ ] **A51** *(filed 2026-08-15)* — PO artifact commit to the main checkout is silently fallible; main-checkout branch/cleanliness invariant enforced only by operator checklist. **Class:** `silent-failure` · **Invariant:** I-5 + I-3. **Evidence:** `_po_flow` runs `git add`/`git commit` on `repo_dir` with `check=False` (`orchestrator.py:369–372`) and never verifies the commit landed; if it fails (hooks, index lock, wrong branch checked out), every subsequent worktree forks from `agent_branch` *without PO context* and the failure surfaces as inexplicable downstream doctrine failures. `fast_forward_target` also force-checkouts `target_ref` in the main checkout (`git_worktree.py:192–197`); PF-6 is the only guard. **Fix:** Batch 7-1 — run-start checkout preflight in code + checked PO commit with post-commit `rev-parse` verification. **Priority:** high.
- [ ] **A52** *(filed 2026-08-15)* — Agent subprocess inherits the full server environment (incl. `AZURE_*`/`OPENAI_*` secrets) under `--dangerously-skip-permissions`, while consuming untrusted target-repo content via retrieval. **Class:** `scope-creep` (trust boundary) · **Invariant:** I-1. **Evidence:** `claude_agent.py:288` (`{**os.environ, ...}`), `:276` (skip-permissions), secret passthrough list at `:157–159`; A47 already established `--allowedTools` is not a boundary. Prompt injection in a brownfield target ⇒ arbitrary shell with operator credentials. **Fix:** Batch 7-3 — env allowlist (`HOME, PATH, GIT_*, CLAUDE_*` + operator-extendable), trust-model section in `HARNESS.md`; full sandboxing deferred to its own plan (operator decision D4). **Priority:** medium for self-operated use; **blocking** for org adoption.
- [ ] **A53** *(filed 2026-08-15)* — Mid-sprint indexer results are never checked: `_run_indexers` ok-flags are yielded into the stream and consulted by no code path. **Class:** `silent-failure` · **Invariant:** I-5. **Evidence:** `orchestrator.py:1827, 1939, 2003` — if Milvus dies post-preflight (A3 covers preflight only), every reindex fails silently and QA/engineers ground against stale embeddings while Tier 1.5 counts their calls as "grounded." The crew's defining property — groundedness — has no runtime health check. **Fix:** Batch 7-2 — check ok; one mid-sprint Milvus-restart + re-run; still failing → infra route (pause/triage), never silent continuation. **Priority:** high.
- [ ] **A54** *(filed 2026-08-15)* — `has_new_commits(wt, base_ref="HEAD~1")` counts "HEAD has a parent" (always ≥1), not "agent committed." **Class:** `enforcement-gap` · **Invariant:** I-2. **Evidence:** `orchestrator.py:468,618` → `git_worktree.py:227`; currently masked by doctrine no-op detection but semantically wrong — a validator refactor could silently expose it. **Fix:** Batch 2-4 — record base SHA at `create_worktree`; count `<base_sha>..HEAD`. **Priority:** low.
- [ ] **A55** *(filed 2026-08-15)* — `extract_section` returns `None` on BL-heading format drift and the literal string `"None"` is interpolated as the task into engineer/QA/scorer prompts. **Class:** `silent-failure` · **Invariant:** I-5. **Evidence:** `backlog.py:76–83` → `_resolve_engineer_section` (`orchestrator.py:384–404`) → f-string at `prompts_brownfield.py:281`. No guard anywhere. **Fix:** Batch 2-4 — structured `backlog_section_missing` failure event instead of a garbage prompt. **Priority:** medium.
- [ ] **A56** *(filed 2026-08-15)* — No `--model` pinning on agent spawns: crew behavior and rubric calibration silently change with the operator's CLI default; B14 `harness_sha` captures code identity but not model identity. **Class:** `observability-gap` · **Invariant:** I-4. **Evidence:** `claude_agent.py:273–282` (no model arg). **Fix:** Batch 2-4 — `AGENT_MODEL` config → `--model` flag; model id recorded in trace `meta.json`. **Priority:** medium.
- [ ] **A57** *(filed 2026-08-15)* — Retrieval-budget breach (call #31) hard-kills the agent mid-work instead of degrading gracefully. **Class:** `resource-leak` (of work, not memory) · **Invariant:** I-1. **Evidence:** `claude_agent.py:479–496` — R8's enforcement discards in-flight work; the retry respawns in the same worktree but the in-context reasoning is lost. **Fix:** Batch 2-4 — enforce budget MCP-server-side (budget-exhausted *error result*; agent continues with what it has); harness kill retained as 2× backstop. **Priority:** low.
- [ ] **A58** *(filed 2026-08-16, C-1 abort smoke)* — Agent-spawned bare `docker run` containers escape every reaper. **Class:** `resource-leak` · **Invariant:** I-1/I-3. **Evidence:** C-1 sprint `run-20260816T150903Z-238ba0` aborted mid-QA; cleanup verified clean EXCEPT one leftover container `qa-bl0001-pg` — the QA agent had launched postgres via bare `docker run --name ...` (not compose). All three A48 reapers (worktree compose reaper, shutdown reaper, volume reaper) sweep by compose project labels; an unlabeled `docker run` container is invisible to all of them, and closure_check's container scan filters on the harness's own label/name prefixes. **Fix directions:** (1) SKILLS clause (engineer/QA): containers must be started via `docker compose` in the worktree (label-swept) — never bare `docker run`; (2) structural: extend `_reap_worktree_compose_stacks` pass 2 with a name-prefix sweep for containers matching the agent's bl/task id; (3) closure_check candidate scan for containers created during the run window. **Priority:** low-medium (small, observable, manual reap trivial) — but it is the first live counterexample to "A48 closed the leak classes."
- [x] **A59** *(filed + fixed 2026-08-16, found during C-1)* — Retrieval MCP server fails to start → **every agent runs ungrounded, silently**. **Class:** `silent-failure` · **Invariant:** I-2 (the doctrine's grounding floor is unenforceable if the tools never exist) + I-5. **Evidence:** C-1 sprint `run-20260816T150903Z-238ba0`. No `retrieval.jsonl` in ANY of the three agent traces; engineer trace `20260816T151431Z-engineer-BL-0001` shows **0 `mcp__retrieval__*` tool_use calls** and terminal phases `['spawn','pre_grounding_violation','exit']`. Two independent root causes, both from the machine migration: (1) `mcp` was absent from the rebuilt venv (requirements.txt only pinned fastapi/uvicorn/pydantic), and (2) once installed, `from __future__ import annotations` in `retrieval_server.py` made FastMCP's `issubclass(param.annotation, Context)` raise `TypeError` on EVERY `@mcp.tool()` registration — so the server aborts at import and the agent silently receives zero retrieval tools. **What worked:** Tier 1.5 fired correctly — `pre_grounding_violation` killed the engineer before it could Write ungrounded. The guard is the only reason this surfaced rather than producing a plausible, unfounded commit. **Second finding (A47 5th instance):** deprived of `mcp__retrieval__*`, the agent reached for `mcp__claude-context__{search_code,index_codebase,get_indexing_status}` — tools from the operator's GLOBAL Claude Code MCP config that were never in our `--allowedTools`. Confirms again that the allowlist is not a containment boundary, AND that agents can silently fall back to an unaudited retrieval path whose calls never enter `retrieval.jsonl` (so R5 accounting would be blind to them). **Fix shipped:** `mcp>=1.9,<2` pinned in requirements.txt (2.0.0 removed `mcp.server.fastmcp`); future-import removed from `retrieval_server.py` with a comment naming this failure; new PF-11 preflight check that actually starts the server and asserts the tool list. **Priority:** critical — it invalidates the grounding premise of every sprint, and would have silently poisoned the entire C-2..C-4 calibration ladder had Tier 1.5 not bitten.
