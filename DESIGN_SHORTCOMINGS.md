...
- [~] **A45** *(partial fix shipped 2026-06-05; commit `8005dda`)* — idle-timeout kills a busy/rate-limited agent. **New live evidence (`search_and_discovery_2` BL-0006):** the frontend BL's retry logged **28 `rate_limit_event`s** then went **silent 600s → idle-kill** (it was in API backoff, not hung), wedging the run. **Fix shipped (the in-flight half):** `claude_agent._inflight_readline_timeout()` — while a `tool_use` is in flight (long Bash gate run, or rate-limit backoff with no stream output) the loop uses the WALL timeout, not idle; tool ids tracked via tool_use→tool_result. Tests: `test_a45_a51_agent_guards.py`. **Still open:** the orchestrator wedge half (#3 below) — on retry exhaustion/idle the engineer flow must always emit `bl.done(engineer_unmerged)` and never leave a 0-procs-no-terminal wedge.
- [ ] **A39 — live evidence bump (2026-06-05).** Reproduced in `search_and_discovery_2` BL-0006: gate emitted `regressions: ['tests/playwright::e2e_suite']` ("1 regression") while the raw tail held **5 distinct playwright failures** (`search.spec.ts:13/28/143/62`, `reset-password.spec.ts:30`). The engineer's retry had no failing-test names → ran the gate itself → A45 idle-kill → wedge. **Required (deferred to next session):** `regression_gate._extract_playwright_failures(raw_tail)` (regex `\[chromium\]\s*›\s*(tests/\S+:\d+:\d+)\s*›\s*(.+?)\s*(?:─|$)`) and expand `tests/playwright::e2e_suite` in `regressions[]` into the real node-ids. **Priority bumped to HIGH** — A39 is the upstream cause of the BL-0006 wedge (A39 → self-run-gate → A45). **A49** also re-confirmed: the `reset-password` failure was the recurring flaky one; fix = playwright `--retries` in the gate template (deferred).
