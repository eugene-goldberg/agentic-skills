...
| R21 | every BL declares a structured dependency DAG + the interface contracts it exposes/consumes across BLs | post_validation (`validate_po` → `backlog.dependency_report`/`contract_report`) | wave-execution **Phase 1** (`PROPOSAL_PARALLEL_WAVE_EXECUTION.md`); per BL a `**Dependencies:**` BL-id list (the DAG the future wave scheduler reads) + `**Exposes:**`/`**Consumes:**` contracts; a missing-field / dangling-ref / self-loop / cycle, or a consumed interface no declared dependency exposes, fails the PO gate → existing doctrine retry loop re-prompts the PO (`build_deps_contracts_fix_prompt`, allowed to edit BACKLOG.md). **Artifact + validation only — changes NO execution path; the sequential loop is unchanged (the degenerate 1-BL-per-wave case).** |
