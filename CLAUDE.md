...
| R20 | acceptance live-verifies every acceptance criterion | orchestrator (`_unverified_criteria` in `_acceptance_flow`) | chain 3-4/4; acceptance emits per-criterion `ac_coverage` in report.json; any criterion not `verified` → `criterion_unverified` anomaly → non-clean + blocks `integrity_ok`; every detected failure auto-dispatches (cap raised 1→25, no silent drop) |
