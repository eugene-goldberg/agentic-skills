...
| R22 | a feature's interface contract is materialized as compilable C# stubs (OpenAPI 3.1, HTTP seam) **before** any consuming slice runs | orchestrator (`contract.contract_report` + `dotnet build`) | Contract-First Decomposition **Phase 1** (`PROPOSAL_CONTRACT_FIRST_DECOMPOSITION.md`); the **crew authors** the OpenAPI 3.1 contract + the compilable C# stubs (interfaces / `record` DTOs / controller skeletons w/ `NotImplementedException` + DI regs), grounded in the target's conventions — **no external codegen tool**. R22 gate = structural OpenAPI validation + stubs `dotnet build` green + per-operation conformance; failure drives the no-abort fix loop. The full Contract-First program (Phases A–E: keystone dep-semantics, fan-out doctrine + `dag_width` metric, engineer-builds-against-stubs/mocks, barrier binding `_contract_bind`, cross-layer DI) is LIVE-PROVEN over 4 runs on the C# ecommerce target. **`contract_first` flag is DEFAULT ON (operator 2026-06-16) but the orchestrator FORCES it OFF on non-.NET targets (`_is_dotnet_target` — the materializer/binder are C#/`dotnet`-specific); on non-.NET targets the path is byte-identical to flag-off.** |
