...
| **C — provenance + live calibration smoke** | Log injected lessons to the role's retrieval/trace log (provenance for Stage 2 efficacy measurement); operator runs one sprint with `inject_lessons=true` on a target with prior confirmed findings, confirms blocks render + no regression. | operator-gated smoke; flag-flip proposal on clean result |
